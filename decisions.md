...
## 8. What We Would Do Differently (Future Improvements)

While we are incredibly proud of the depth achieved in 5 days, building for true scale always reveals areas for improvement. If we were to take this to production next week, here is what we would change:

1.  **Swap Celery for Temporal.io:** While Celery and Redis handled background task queuing effectively, orchestrating complex state machines (like our 5-stage pipeline) is difficult. Temporal natively handles exponential backoffs, distributed state, and saga patterns far better than chaining Celery tasks.
2.  **Multi-Tenant Data Isolation:** We deliberately scoped this out to focus on the core extraction logic. However, day-one production readiness requires strict isolation. We would implement Postgres Row-Level Security (RLS) and Elasticsearch filtered aliases to ensure users can never query another tenant's vectors or documents.
3.  **WebSockets for Bidirectional Agent Interactions:** We used Server-Sent Events (SSE) for streaming the AI's response to the frontend. This was lightweight and highly effective. However, if we wanted to expand the Agent to actively *ask* the user clarifying questions mid-query (Human-in-the-Loop), we would upgrade to bidirectional WebSockets to handle the full conversational state without dropping connections.
4.  **Multi-Modal Vision Extraction:** Currently, the system relies on text-extraction libraries to parse the PDF before handing the text to the LLM. In the future, we would upgrade the pipeline to pass the raw document images directly into a vision-language model (like Gemini 1.5 Pro) to preserve complex spatial layouts, tables, and handwritten signatures.
5.  **Event Streaming (Kafka):** For enterprise-scale ingestion (millions of documents), Redis-backed Celery queues can experience memory pressure under massive backlogs. We would introduce Kafka as an event streaming backbone to decouple ingestion from processing, allowing us to throttle and replay events safely.
6.  **Self-Healing Infrastructure:** We would introduce a continuous reconciliation worker that routinely audits the Postgres source-of-truth against the Elasticsearch indices to automatically heal any "split-brain" scenarios where a document exists in the DB but failed to index in ES due to transient network drops.
